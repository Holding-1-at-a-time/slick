...
const ReportsPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const customersData = await ctx.runQuery(api.customers.getAll);
            const vehiclesData = await ctx.runQuery(api.vehicles.getAll);
            const jobsData = await ctx.runQuery(api.jobs.getAll);
    ...
    useEffect(() => {
        const fetchServices = async () => {
            const servicesData = await ctx.runQuery(api.services.getAll);
    ...
    const servicePerformance = useMemo(() => {
        ...
        return Object.entries(performance).map(([serviceId, data]) => ({
            service: services.find(s => s._id === serviceId),
            ...data
        })).filter(item => item.service).sort((a, b) => b.revenue - a.revenue);
    }, [filteredJobs, services]);

    const technicianLeaderboard = useMemo(() => {
        const stats: { [key: string]: { completedJobs: number; revenue: number } } = {};
        technicians.forEach(t => { stats[t._id] = { completedJobs: 0, revenue: 0 }; });
